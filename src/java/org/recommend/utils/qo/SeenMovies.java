...
{
    Connection conn;
    ArrayList seenMovies;
    int groupID;
    
    public SeenMovies(int id)
    {
        groupID=id;
        seenMovies=new ArrayList();
        ...
        try 
        {
            ...
            while(rs.next())
            {
                seenMovies.add(rs.getInt(1));
            }            
        }catch (SQLException ex) 
        {
            System.out.println("Cannot get seen movies");            
            Logger.getLogger(MovieRatingsDistinct.class.getName()).log(Level.SEVERE, null, ex);            
        }         
    }
    
    public ArrayList getSeenMovies()
    {
        return seenMovies;
    }
}
